...
def initial_state():
    ...
    # maps training input x_i to associated weight beta_i:
...
def predict(state, kernel, x_i):
    ...
    # *** START CODE HERE ***
    z = 0
    for x_j in state.keys(): 
        beta = state[x_j]
        z += beta * kernel(np.asarray(x_j), x_i)
    return sign(z)
    # *** END CODE HERE ***
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    """
    # *** START CODE HERE ***
    beta_update = learning_rate * (y_i - predict(state, kernel, x_i))
    state[tuple(x_i)] += beta_update
    # *** END CODE HERE ***
