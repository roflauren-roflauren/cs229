...
2. Every problem has its own directory (*e.g.,* `tex/featuremaps/` for Problem 3).
3. Every subproblem has two files within the parent problem’s directory:
  - The problem statement (*e.g.* `tex/featuremaps/01-degree-3-math.tex` for Problem 3(a)). You do not need to modify this.
  - Your solution, *e.g.* `tex/featuremaps/01-degree-3-math-sol.tex` for your solution to Problem 3(a). You will need to modify these files (and the source files in `src` for coding parts).
